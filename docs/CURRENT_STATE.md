...
Cập nhật lần cuối: sau **M11-COMPOSE** — LLM compose chuỗi rule boolean lồng qua trung gian trên đường generic (không module mới); validator cấm trùng rule target (2 tầng); probe `nested_boolean`; vòng lặp biến tự do từ chối trung thực (gate fired).
...
| vitest | **325 pass** (0 network call; +2 do M11: trùng-target + chuỗi lồng hợp lệ ở validator TS) |
