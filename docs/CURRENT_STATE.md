...
| | |
|---|---|
| pytest | **282 pass** (0 API call thật — guard là bằng chứng) |
| vitest | **150 pass** (0 network call) |
...
8. **[M8-PRE — CHƯA XỬ LÍ, CẦN QUYẾT ĐỊNH] Cảnh SƠ ĐỒ HỆ THỐNG hay vượt
   `max_objects = 20`.** Đề "phân tích hệ thống" giờ **định tuyến đúng** về
   `generic.rule_scene` (hết từ chối im lặng), nhưng stage simulate **thường
   thất bại vì vượt hạn mức object** → API trả 422 "chưa sinh được cấu hình hợp lệ".
   Nguyên nhân đo được (3 lần diag, đếm từng attempt): Gemini tạo **một object
   `label` riêng cho mỗi node/cạnh** (có attempt 11 label) cộng heading/container
   trang trí → 22–27 object. Sửa prompt/contract **KHÔNG hiệu quả** (đã thử 2 vòng,
   đúng như bài học của `directed`).
   **Bản chất: hạn mức 20 được chọn cho cảnh nhỏ (cổng logic, tam giác), không đủ
   cho sơ đồ 6 thành phần + 6 luồng.** Hai lựa chọn:
   (a) **Nâng `limits.max_objects` trong manifest** (vd 20 → 28) + mirror hằng số
       `MAX_OBJECTS` ở `frontend/.../generic/validate.ts`. Một con số, dẫn xuất
       khắp nơi. **Đổi ngữ nghĩa DSL v1 cho MỌI cảnh** → cần duyệt tường minh.
   (b) Giữ 20 và chấp nhận cảnh hệ thống lớn thất bại (đề nhỏ vẫn chạy).
   **Chưa tự ý làm (a)** vì nó vượt scope S2 đã duyệt và ảnh hưởng toàn DSL.
