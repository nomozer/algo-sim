...
Cập nhật lần cuối: sau **M13-SOUNDNESS Task 1–14 + hotfix role-compat — ĐÃ
MERGE FF vào `main`** (`db5ba3f`→`e8c9dba`). Task 14 live smoke ĐÃ CHẠY (user
...
1. **`_simulate_with_metrics` (harness) mirror `stage_simulate` (pipeline)** —
   nguy cơ drift; chưa hợp nhất. **[M14 discovery — đo đạc cụ thể]** Drift đã
   vượt "nguy cơ": (a) harness KHÔNG gọi `run_pipeline` mà tái dựng stage riêng
   (`evaluate_item`); (b) KHÔNG chạy `check_semantic_compatibility` TRONG vòng
   retry như production (`pipeline.py` stage_simulate) — LLM trong eval không
   nhận feedback thiếu-role; (c) KHÔNG gọi `check_computation_ownership`
   (M13 gate B) — xem 7d; (d) `classify_error` phân nhóm lỗi bằng string-match
   message tiếng Việt của validator — mirror mong manh đã cắn thật (7f).
   Hợp nhất production/evaluation lifecycle là **target bắt buộc của M14**.
