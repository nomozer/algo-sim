...
Cập nhật lần cuối: sau **M13 Task 1–13** trên nhánh `m13-semantic-soundness`
(CHƯA merge vào `main`, CHƯA hoàn tất milestone) — Task 13 chỉ verify offline
toàn cục + cập nhật docs; **Task 14 (live smoke có mục tiêu) đang khoá, chờ
user phê duyệt tường minh `ALLOW_LIVE_AI=1`**. Xem hàng **M13-SOUNDNESS** ở §2
để biết M13 giải quyết gì. Trước đó: sau **M12-AI-SCAN** (tiếp nối
...
7d. **[M13] `gap_gate_recall` (harness) chỉ phản ánh KÊNH 1 của `computation_gate.py` (known-gap roles lọt vào `unsupported_capabilities`), CHƯA phản ánh KÊNH 2 (`result_ownership` fail-closed).** Đây là **giới hạn của metric**, không phải giới hạn của gate thật: outcome mỗi case eval vẫn đi qua `run_pipeline` thật (cả hai kênh cùng sống ở đó), metric chỉ **đo/báo cáo** dựa trên kênh 1 để số liệu còn so sánh được với baseline M7.14T. Nếu cần đo riêng đóng góp của kênh 2 (case gap CHỈ do `result_ownership="algorithmic"` bắt được, kênh 1 im) → cần một metric mới, chưa làm trong M13.
...
7f. **[M13 Task 14] `m11-nested-canonical` đỏ ×2 live cùng chữ ký `unknown_primitive`** (invalid 3 attempt, cả run suite lẫn rerun chẩn đoán riêng). Bằng chứng KHÔNG phải M13 chặn oan: (a) trace `classify_error` — message các đường chặn M13 (coherence "không có nguồn giá trị"/role "vai trò"/target "không nhận được") rơi vào nhóm khác, `unknown_primitive` chỉ khớp "object type"/"type không hợp lệ"; (b) enum object/interaction/process trong `_GENERIC_SCHEMA` dẫn xuất từ manifest, M13 không đụng; (c) case này flaky có hồ sơ từ M11 prompt cũ (✅→✅→❌ qua các run). **Nguyên nhân gốc CHƯA chốt được** vì harness trước đây VỨT message lỗi validation (`detail` rỗng) — đã vá tại `harness.py` (`_simulate_with_metrics` trả kèm message, commit `c3a11b9`). Follow-up: cần ngân sách nhỏ (~4-6 call) rerun canonical để dump message thật rồi mới quyết vá prompt; **quyền vá-prompt-1-lần của Task 14 CHƯA dùng**. 4 case m11 còn lại + trọn m12 + Dijkstra đều xanh — không phát sinh FP nào từ M13.
