...
| `construct_section` hỏng khi mặt phẳng cắt **chứa trọn một CẠNH** của khối | **OPEN — `SECTION_COPLANAR_EDGE_GAP`** | `MALFORMED_SOLID: thiết diện không khép kín`. Chạm (SAC), (SBD), mặt chéo ACC′A′ — ba mặt phẳng phổ biến bậc nhất. Đo lại 2026-09-02 (0 call, `GEOMETRY_ARCHITECTURE_EXPRESSIVENESS_AUDIT §18`): biến quyết định là **số cạnh nằm trong mặt phẳng cắt**, không phải số đỉnh — 3 đỉnh trên mặt phẳng mà 0 cạnh thì chạy đúng. *Trước đây xếp nhầm là `SECTION_VERTEX_INTERSECTION_GAP` ("hỏng khi qua một đỉnh"), mô tả ấy SAI.* Cố ý KHÔNG sửa runtime trong wave này | chưa xếp |
