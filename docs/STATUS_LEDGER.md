...
| `angle_cos_sq` trả **sin²** cho cặp (đường, mặt) | **OPEN — bug thật** | `fresh-probe fp_5`: mô hình đặt biến `cos_angle_SC_ABC_sq`, nhận 1/3 = sin²; oracle 2/3. Tên ở tầng IR nói dối; `measure_contract.nghia` hiện cũng ghi sai. Đổi ngữ nghĩa một phép đo = đổi năng lực ⇒ cần wave riêng + bộ đề mới | chưa xếp |
| Bảng nghĩa vụ trong prompt quảng cáo từ vựng KHÔNG viết được | **OPEN — bug thật** | `fresh-probe fp_6`: mô hình phát `{"kind":"perpendicular"}` cả hai lượt rồi hết ngân sách. `SemanticProgramSpec` không có trường `obligations`; nghĩa vụ thuộc phía hợp đồng. Cùng lớp lỗi với nhãn `construct_plane.through` | chưa xếp |
