...
| **IR thiếu phép TỊNH TIẾN ĐIỂM** | **OPEN — khuôn hỏng chi phối, đã định lượng** | 9/9 lượt hỏng của k=3 đều là SCHEMA, và **10 lần cùng một hình dạng**: `construct_point X = arith(+, var(P), vector_from_points(A,B))` — *"tịnh tiến P theo vectơ AB"*. Đó là cách duy nhất tự nhiên để dựng `C` hình bình hành và `B'`,`C'`,`D'` của lăng trụ/hộp; `PointExpr` có 5 phép, không phép nào là tịnh tiến. Mô hình đang cố **DỰNG** thay vì **KHAI** — tôn trọng R0 chặt hơn thứ IR cho phép. `grounding_gate` Wave 5 đã ghi khoảng trống này rồi bỏ quên. Hai đề không cần tịnh tiến (`v2_01`, `v2_06`) đều 3/3 STABLE | chưa xếp |
