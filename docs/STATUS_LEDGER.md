...
| `construct_section` hỏng khi mặt cắt qua ĐỈNH khối | **OPEN — `SECTION_VERTEX_INTERSECTION_GAP`** | `MALFORMED_SOLID: thiết diện không khép kín`. Tìm ra lúc kiểm khả biểu diễn (0 call), trước khi tiêu quota. Cố ý KHÔNG sửa trong wave first-binding | chưa xếp |
