...
|---|---|---|---|--:|---|
| **A01** | intersection | construction | `point_on_line` | 3 | ✅ · Giao tuyến hai mặt phẳng — điểm thuộc giao tuyến |
| **A02** | point_construction | verdict | `point_on_plane` | 3 | ✅ · Điểm thuộc mặt phẳng |
| **A03** | line_relation | verdict | `parallel` | 2 | ✅ · Hai đường thẳng song song |
| **A04** | line_relation | verdict | `parallel` | 3 | ✅ · Đường thẳng song song mặt phẳng |
| **A05** | plane_construction | verdict | `parallel` | 2 | ✅ · Hai mặt phẳng song song |
| **A06** | line_relation | verdict | `perpendicular` | 2 | ✅ · Hai đường thẳng vuông góc |
| **A07** | line_relation | verdict | `perpendicular` | 2 | ✅ · Đường thẳng vuông góc mặt phẳng |
| **A08** | plane_construction | verdict | `perpendicular` | 2 | ✅ · Hai mặt phẳng vuông góc |
...
| **B02** | measurement | refusal | `—` | 1 | ✅ · Khoảng cách đường ∥ mặt, hoặc mặt ∥ mặt |
| **B03** | measurement | refusal | `—` | 2 | ✅ · Góc nhị diện có miền (có thể tù) |
| **B04** | — (không họ nào khớp) | refusal | `—` | 2 | ✅ · Oxyz: viết phương trình mặt phẳng / đường / mặt cầu |
| **B05** | solid_geometry | refusal | `—` | 2 | ✅ · Mặt cầu · mặt nón · mặt trụ |
| **B06** | line_relation | refusal | `—` | 2 | ✅ · Phép toán vectơ, hoặc phép chiếu song song |
...
|---|---|---|---|---|---|--:|---|---|
| **A01** | ≥1 | `intersection_point` | `invariant_relation` → `point_on_line` | ① ② ③a ③b ⑤ | Quan hệ song song Toán 11 (32tr, 0 trắc nghiệm) | 3 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A02** | ≥1 | `incidence` | `predicate_boolean` → `point_on_plane` | ① ② ③a ③b ⑤ | Quan hệ song song Toán 11 (32tr, 0 trắc nghiệm) | 3 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A03** | ≥1 | `parallel_relation` | `predicate_boolean` → `parallel` | ① ② ③a ③b ⑤ | Quan hệ song song Toán 11 (32tr, 0 trắc nghiệm) | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A04** | ≥1 | `parallel_relation` | `predicate_boolean` → `parallel` | ① ② ③a ③b ⑤ | Quan hệ song song Toán 11 (32tr, 0 trắc nghiệm) | 3 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A05** | ≥1 | `parallel_relation` | `predicate_boolean` → `parallel` | ① ② ③a ③b ⑤ | Quan hệ song song Toán 11 (32tr, 0 trắc nghiệm) | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A06** | ≥1 | `perpendicular_relation` | `predicate_boolean` → `perpendicular` | ① ② ③a ③b ⑤ | Quan hệ vuông góc — Lê Minh Tâm (117tr, 0 trắc nghiệm) | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A07** | ≥1 | `perpendicular_relation` | `predicate_boolean` → `perpendicular` | ① ② ③a ③b ⑤ | Quan hệ vuông góc — Lê Minh Tâm (117tr, 0 trắc nghiệm) | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **A08** | ≥1 | `perpendicular_relation` | `predicate_boolean` → `perpendicular` | ① ② ③a ③b ⑤ | Quan hệ vuông góc — Lê Minh Tâm (117tr, 0 trắc nghiệm) | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
...
| **B02** | ≥1 | `out_of_capability` | `rejection_expected` — **bỏ trống** | **từ chối trung thực** — thang KHÁC | bất kỳ — không cần đáp án đúng, chỉ cần đúng LOẠI | 1 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **B03** | ≥1 | `out_of_capability` | `rejection_expected` — **bỏ trống** | **từ chối trung thực** — thang KHÁC | bất kỳ — không cần đáp án đúng, chỉ cần đúng LOẠI | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **B04** | ≥1 | `out_of_capability` | `rejection_expected` — **bỏ trống** | **từ chối trung thực** — thang KHÁC | bất kỳ — không cần đáp án đúng, chỉ cần đúng LOẠI | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **B05** | ≥1 | `out_of_capability` | `rejection_expected` — **bỏ trống** | **từ chối trung thực** — thang KHÁC | bất kỳ — không cần đáp án đúng, chỉ cần đúng LOẠI | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
| **B06** | ≥1 | `out_of_capability` | `rejection_expected` — **bỏ trống** | **từ chối trung thực** — thang KHÁC | bất kỳ — không cần đáp án đúng, chỉ cần đúng LOẠI | 2 | — | đủ tối thiểu; thêm bài để tổng đạt ngưỡng |
...
|---|---|---|--:|---|
| **point_construction**<br>*dựng điểm · điểm thuộc vật* | A02 | — | 3 | — |
| **line_relation**<br>*quan hệ đường–đường, đường–mặt* | A03, A04, A06, A07 | B06 | 11 | — |
