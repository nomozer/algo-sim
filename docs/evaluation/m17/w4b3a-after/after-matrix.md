...
Tổng: **23 target · 12 family**. Đo được trong trình duyệt **14/23** (**9** target chưa có bài mẫu offline ⇒ chỉ đọc được năng lực KHAI BÁO).

Thao tác trực tiếp: **11 đo được** (+1 chỉ khai báo). Cam kết thuật toán: **8 đo được** (+1 chỉ khai báo). Khai `predict`: **11**.
...
| algorithm.bounded_control_flow | bounded_control_flow | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | algorithm.bounded_control_flow | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | hàm/thủ tục và đệ quy — ngoài phạm vi luồng điều khiển hữu hạn; danh sách/mảng, chuỗi, số thực, nhập xuất — chưa có trong ngữ pháp v1 |
...
| algorithm.scan | single_pass_scan | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | algorithm.scan | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | một lượt quét nhiều biến tích luỹ (vd tìm cả max lẫn min) — single_pass_scan.multi_accumulator, BACKLOG |
| algorithm.selection_sort | comparison_sort | PUBLIC_AI_ONLY | không | UNMEASURED_DECLARES_EXPLORE | progressive | algorithm.selection_sort | khai (chưa đo) | khai (chưa đo) | có | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | — |
...
| binary.base_conversion | positional_representation | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | binary.base_conversion | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | — |
| binary.character_encoding | positional_representation | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | binary.character_encoding | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | emoji và ký tự ngoài BMP (mã > 65535) — ngoài phạm vi v1; dãy byte UTF-8/UTF-16, Base64, nén, mã hoá bảo mật — ngoài phạm vi |
...
| database.relational_table_query | relational_table_query | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | database.relational_table_query | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | JOIN nhiều bảng · truy vấn lồng · thêm/sửa/xoá dữ liệu · SQL tự do · GROUP BY nhiều nhóm · kết nối CSDL thật — ngoài phạm vi v1; phần tích luỹ (đếm/tổng/cực trị) TRÙNG cơ chế với single_pass_scan; cái mới ở đây là KHUNG QUAN HỆ (lược đồ, kiểu cột, vị từ trên bản ghi, phép chiếu, sắp xếp ổn định) |
...
| logic.boolean_dag | boolean_composition | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | hybrid | logic.boolean_dag | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | — |
| network.graph_traversal | graph_traversal | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | network.graph_traversal | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | đường đi ngắn nhất có trọng số (Dijkstra) — future family |
...
| tree.traversal | tree_traversal | PUBLIC_AI_ONLY | không | UNMEASURED_HAS_TIMELINE | progressive | tree.traversal | không khai | không khai | không | khai (chưa đo) | 2D_ONLY | 2d | — | KHÔNG ĐO ĐƯỢC (không có bài mẫu offline) | BST/AVL/heap/cây biểu thức/cây n-nhánh — ngoài phạm vi duyệt cây nhị phân |
...
```json
{
  ...
  "experience": {
    "INTERACTIVE_MODEL": 7,
    "UNMEASURED_HAS_TIMELINE": 8,
    "COMMITMENT_TRACE": 2,
    "UNMEASURED_DECLARES_EXPLORE": 1,
    "INTERACTIVE_STAGE": 3,
    "TRACE_PLAYBACK": 1,
    "BOUNDED_ARTIFACT": 1
  },
  "direct_manipulation_measured": 11,
  "direct_manipulation_gated": 7,
  "direct_manipulation_always_on_stage": 4,
  "direct_manipulation_declared_only": 1,
  "algorithm_commitment_measured": 8,
  "algorithm_commitment_declared_only": 1,
  "challenge_predict": 11,
  "measured_in_browser": 14,
  "not_measurable_no_sample": 9,
