> # ⚠️ LEGACY DOCUMENT — KHÔNG PHẢI NGUỒN CHÂN LÝ HIỆN HÀNH
>
> **TÀI LIỆU LỊCH SỬ (thiết kế ban đầu, v0.3). KHÔNG dùng để ra quyết định
> implementation.** Nhiều kiến trúc mô tả dưới đây KHÔNG tồn tại trong hệ
> thống hiện tại: "ba nguồn trace", tầng code Pyodide (Lớp 5), vẽ tự do
> `llm_script` (Lớp 6), `fallback: code/draw`. Kiến trúc hiện hành là
> registry mô-đun + generic DSL + capability gate — đọc `docs/RULES.md`
> (bản hiện hành) và `docs/ARCHITECTURE_MAP.md`. Nếu tài liệu này mâu thuẫn
> với code/test: **CODE/TESTS THẮNG.**
