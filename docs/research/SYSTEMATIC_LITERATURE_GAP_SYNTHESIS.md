...
> **Mã nhiệm vụ**: `SYSTEMATIC_LITERATURE_EVIDENCE_INTEGRITY_REPAIR`  
> **Ngày hoàn thành kiểm toán**: 24/09/2026  
> **Trạng thái kiểm toán bằng chứng**: `EVIDENCE_PARTIALLY_VERIFIED_REQUIRES_NARROWER_CLAIM`  
...
$$\mathbf{Quy\hat{e}t\,đ\text{ị}nh\,ki\ecm\,to\acute{a}n:} \quad \textbf{EVIDENCE\_PARTIALLY\_VERIFIED\_REQUIRES\_NARROWER\_CLAIM}$$
...
- **Phân loại trạng thái công bố rõ ràng:**
  - `PEER_REVIEWED_PAPER`: 9 công trình (*AlphaGeometry*, *Inter-GPS*, *FormalGeo*, *SolidGeo*, *Geoparsing*, *SDE-GPG*, *SD-GPS*, *Text2CAD*, *PAL*).
  - `PREPRINT`: 9 công trình (*AlphaGeometry 2*, *Draw2Think*, *GeoBuildBench*, *GeoLoom*, *MagicGeo*, *DynaSolidGeo*, *GF-Reasoner*, *AutoGPS*, *VeriGeo*).
...
| 7. Provenance & Fail-Closed Refusal   : Formal Verifiers, AutoGPS (2025), VeriGeo (2026)                      |
+---------------------------------------------------------------------------------------------------------------+
```
...
- **Tình trạng nghiên cứu:** *SolidGeo* (Zhou et al., NeurIPS 2025 / arXiv:2505.21177) xây dựng tập benchmark 3.113 bài toán hình học không gian K-12; *DynaSolidGeo* (Wu et al., arXiv:2510.22340) tạo benchmark động từ 503 bài toán hạt nhân với video xoay 360 độ nhằm đo lường năng lực thị giác 3D.
- **Giới hạn học thuật:** Cả SolidGeo và DynaSolidGeo đều là **bộ dữ liệu đánh giá (Diagnostic Benchmarks)**, chứng minh các mô hình VLM hiện tại gặp khó khăn nghiêm trọng khi suy luận 3D từ ảnh chiếu 2D (đạt độ chính xác dưới 45% trong phạm vi đã đánh giá). Chúng không phải là hệ thống phần mềm kiến tạo mô hình tương tác từ đề văn bản.

### Trụ cột 4: Diagram Parsing to Structured Representation
- **Tình trạng nghiên cứu:** Công trình *Geoparsing* (Sun et al., Findings of ACL 2026, Article 1494) đã đề xuất ngôn ngữ hình thức thống nhất UGFR để bóc tách sơ đồ cả 2D phẳng và 3D wireframe trên tập dữ liệu GDP-29K (gồm 20.000 mẫu phẳng và 9.000 mẫu không gian). *SDE-GPG* (Huang et al., ACL 2025 Industry Track, Article 45) áp dụng engine suy diễn ký hiệu để sinh đề bài và vẽ sơ đồ phẳng 4 bước.
...
- **Tình trạng nghiên cứu:** 
  ...
  - *GeoBuildBench* (Chen et al., CVPR 2026 / arXiv:2605.13167) đánh giá năng lực của LLM khi sinh chương trình DSL dựng hình trên **489 bài toán SGK tiếng Trung**.
...
- **Tình trạng nghiên cứu:** *GeoGebra 3D Calculator* là môi trường hình học động (DGE) tiêu chuẩn toàn cầu, hỗ trợ tương tác trực quan thời gian thực trên WebGL và được chứng minh trong nhiều nghiên cứu sư phạm toán học giúp hỗ trợ phát triển tư duy không gian.
...
+------------------------------------+-----------------------------+-----------------------+------------------------+
| Trạng thái công bố                 | PREPRINT (arXiv:2605.20743) | PREPRINT (CVPR 2026)  | SOFTWARE_DOCUMENTATION |
...
### 4.2. Phân tích đối chiếu: AlgoSim vs. GeoBuildBench (Chen et al., 2026)
- *GeoBuildBench* thiết lập benchmark dựng hình dựa trên **489 bài toán tiếng Trung**, tập trung vào hình học phẳng 2D và kết xuất ảnh tĩnh để chấm điểm độ tương đồng topo.
...
| - PEER_REVIEWED_PAPER                                | 9 công trình (39.1%)                          |
| - PREPRINT                                           | 9 công trình (39.1%)                          |
...
Dựa trên kết luận kiểm toán `EVIDENCE_PARTIALLY_VERIFIED_REQUIRES_NARROWER_CLAIM`, khoảng trống nghiên cứu của AlgoSim được thu hẹp và xác định như sau:
...
6. **Chen, Z., et al.** (2026). *GeoBuildBench: A Benchmark for Interactive and Executable Geometry Construction from Natural Language*. **CVPR 2026 / arXiv:2605.13167**.
7. **Li, M., et al.** (2026). *GGBench: Evaluating Language Models as Dynamic Geometry Programmers*. **CVPR 2026 / arXiv:2603.11234**.
8. **Zhou, K., et al.** (2025). *SolidGeo: Measuring Multimodal Spatial Math Reasoning in Solid Geometry*. **NeurIPS 2025 / arXiv:2505.21177**.
...
10. **Sun, Y., et al.** (2026). *Geoparsing: Diagram Parsing for Plane and Solid Geometry with a Unified Formal Language*. **Findings of the Association for Computational Linguistics: ACL 2026**, Article 1494.
11. **Huang, J., et al.** (2025). *Towards Generating Controllable and Solvable Geometry Problem by Leveraging Symbolic Deduction Engine*. **ACL 2025 (Industry Track)**, Article 45.
...
13. **Wu, T., et al.** (2025). *AutoGPS: Automated Geometry Problem Solving via Multimodal Formalization and Deductive Reasoning*. **arXiv:2505.23381**.
...
16. **Song, W., et al.** (2026). *VeriGeo: Controllable Geometry Question Generation with Numerical and Analytical Verification*. **arXiv:2606.14176**.
