...
### Task 2b: Gỡ `object.weight` — silent semantic no-op (duyệt 2026-07-17)

**Bối cảnh:** Task 1 matrix F3 reclassify `OUT-OF-SCOPE` → `UNSUPPORTED_SEMANTIC_FIELD`. Field `weight` cấp OBJECT được **quảng bá trong public schema + DẠY trong contract prompt + validate + patch được**, nhưng **không runtime nào đọc** — trọng số thật luôn là `rule.weights`. Đúng lớp bug §9b. Xử lí = **GỠ**, không thêm ngữ nghĩa. Đặt TRƯỚC Task 3/5 vì sửa đúng dòng normalization mà Task 3/5 sẽ sửa.

**Files:**
- Modify: `backend/app/simulation/catalog.py:384` (bỏ `"weight"` khỏi `_GENERIC_SCHEMA`)
- Modify: `backend/app/simulation/dsl/manifest.py:321` (contract text: bỏ "(có weight)")
- Modify: `backend/app/simulation/dsl/validator.py:263` + `frontend/src/simulations/domains/generic/validate.ts:217` (tuple normalization: bỏ `"weight"`; thêm reject legacy)
- Modify: `backend/app/simulation/patch.py:40` + `frontend/src/simulations/domains/generic/patch.ts:37` (bỏ `"weight"` khỏi `ADD_FIELDS`)
- Modify: `frontend/src/data/sim-samples.ts:123-126` (bỏ 4 field `weight:` khỏi `GENERIC_BINARY_SPEC`; **GIỮ NGUYÊN** `rule.weights: [8,4,2,1]`)
- Test: `backend/tests/test_dsl.py`, `frontend/src/simulations/domains/generic/generic.test.ts`

**Interfaces:**
- Produces: cả hai validator từ chối config chứa `object.weight` với thông điệp tiếng Việt chứa cụm `không còn được hỗ trợ`. KHÔNG strip im lặng.
- **KHÔNG** thêm accessor trọng-số-cạnh, KHÔNG ngữ nghĩa mới. `rule.weights` không đụng.

- [ ] **Step 1: Viết test fail (backend)** — thêm vào `test_dsl.py`:
...
- [ ] **Step 2: RED** — `cd backend && .venv/Scripts/python -m pytest tests/test_dsl.py -v -k "object_weight or binary_weighted_sum"` → test 1 FAIL (validator hiện chấp nhận + giữ weight).

- [ ] **Step 3: Cài (backend)** — (a) `catalog.py:384`: xoá dòng `"weight": {"type": "NUMBER", "nullable": True},`. (b) `manifest.py:321`: `"Đổi nhị phân = switch bit (có weight) + value_box + rule weighted_sum. "` → `"Đổi nhị phân = switch bit + value_box + rule weighted_sum (weights là mảng TRÊN RULE, vd [8,4,2,1]). "`. (c) `patch.py:40`: bỏ `"weight"` khỏi `ADD_FIELDS`. (d) `validator.py:263`: đổi tuple thành `("x", "y", "value")` và thêm reject TRƯỚC vòng copy (đặt trong vòng lặp duyệt object, `o` là object thô):
...
- [ ] **Step 4: GREEN backend** — 2 test PASS. Chạy `python -m pytest` toàn suite: test nào dùng `object.weight` trong fixture sẽ đỏ → **sửa fixture (bỏ field)**, đó là migrate đúng chủ đích, KHÔNG nới validator.

- [ ] **Step 5: Frontend mirror + migrate sample** — test vitest mirror 2 case trên (reject `không còn được hỗ trợ`; sample nhị phân không `weight` → `valuesOf` ra `out === 13`); `validate.ts:217` tuple → `["x", "y", "value"]` + reject cùng thông điệp; `patch.ts:37` bỏ `"weight"`; `sim-samples.ts:123-126` xoá `weight: 8/4/2/1` (giữ `value`, `x`, `y`, `label`, và `rule.weights`).

- [ ] **Step 6: GREEN frontend** — `npx vitest run -t "weight"` PASS; `npm test` toàn bộ PASS; `npm run build` sạch. **Kiểm chứng sample thật**: `GENERIC_BINARY_SPEC` sau migrate vẫn validate + `out === 13` (test Step 5 đã khoá).

- [ ] **Step 7: Commit** — `M13 Task 2b: gỡ object.weight (silent semantic no-op) — schema/contract/normalization/patch-allowlist/sample; validator từ chối legacy, không strip im lặng`

**Ghi chú hệ quả đã lường trước:** lịch sử localStorage chứa envelope bài mẫu nhị phân CŨ (có `switch.weight`) sẽ fail-closed khi mở lại — đúng máy móc M13 (Task 6 store fail-closed), thông báo trung thực, học sinh mở lại bài từ Thư viện. Đây là **quyết định đã duyệt**, không phải bug.
...
- [ ] **Step 5: Regression lock cho nhánh đang mổ (duyệt 2026-07-17)** — Task 3 mổ `validate_generic_config`; matrix §8 ghi ba nhánh đúng-nhưng-chưa-test nằm trong chính hàm này. Khoá trước khi sửa tiếp (mỗi test PASS ngay — đây là lock, không phải RED):

```python
def test_move_along_path_ref_id_khong_phai_node_bi_reject():
    """matrix §8.1 — validator.py:434-436 đã chặn đúng, nay khoá lại."""
    spec = _spec(
        objects=[
...
(`_spec` cần nhận `processes=` — nếu helper hiện tại chưa có tham số đó, thêm với default `None` → `[]`.)

- [ ] **Step 6: Commit** — `M13: validator backend — operand coherence ba trạng thái (INVALID_SOURCE reject, derived defer) + lock 3 nhánh matrix §8`
...
- [ ] **Step 5: Regression lock frontend (duyệt 2026-07-17)** — Task 5 mổ `validateGenericConfig`; khoá nhánh matrix §8 phía frontend (PASS ngay, là lock):

```ts
it("M13: toggle nhắm vào target của rule bị từ chối (giá trị dẫn xuất không toggle được)", () => {
  const res = validateGenericConfig({
    version: "1.0", title: "t",
    objects: [
...
Cộng hai mirror của Task 3 Step 5: `move_along_path` path trỏ id không-phải-node → reject; `edge.from/to` trỏ object không tồn tại → reject.

- [ ] **Step 6: Commit** — `M13: validator frontend tiêu thụ dsl-contract.json — operand coherence + role-typing parity backend + lock nhánh matrix §8`
