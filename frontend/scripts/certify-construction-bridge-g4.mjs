 ...
 * ─── ĐIỀU NÓ BẢO VỆ ───────────────────────────────────────────────────────
 *
 * `plane_perpendicular_to_line` là phép DUY NHẤT được thêm ở G4: ba phép cùng
 * nhóm trong kernel đã diễn đạt được bằng IR cũ, nên chúng không có cửa riêng.
 * pytest chứng minh chữ ký, tính chính xác, xuất xứ và vết; lượt này trả lời
 * câu chỉ trình duyệt trả lời được — **học sinh có thấy mặt phẳng ấy không**.
 *
 * Bốn điều phải đúng cùng lúc: cảnh dựng được · mặt phẳng đi qua **tuyến vẽ
 * CŨ** (`surface`, không loại vẽ mới) · ô soi nói đúng xuất xứ · và tên hiển
 * thị là một câu tiếng Việt chứ không phải `plane_perpendicular_to_line`.
 *
 * ⚠️ Bài ở THƯ VIỆN, không ở bộ gợi ý trang chủ; cây thành phần nằm trong một
 * ngăn đóng mặc định. Cả hai bẫy đã cắn ở lượt trước — xem
 * `certify-section-coplanar-edge.mjs`.
 */
...
const mo = await evj(`(()=>{const c=[...document.querySelectorAll('.sample-card,.session-card,article,li,button,a')]
  .filter(e=>/vuông góc với đường thẳng cho trước/i.test(e.textContent||''));
  const n=c[c.length-1]; if(n) n.click(); return JSON.stringify({co:!!n});})()`);
kt("§35 thư viện có bài dựng mặt phẳng vuông góc", JSON.stringify(mo), !!mo?.co);
await sleep(3200);
...
let t = await T();
kt("§35 xưởng 3D dựng được", `canvas=${t.canvas} duPhong=${t.duPhong}`,
   t.canvas === 1 && !t.duPhong);
kt("§19 mặt phẳng mới mang TÊN tiếng Việt, không mang tên phép dựng",
   JSON.stringify(t.cay.filter((x) => /vuông góc/i.test(x))),
   t.cay.some((x) => /Mặt phẳng qua .* vuông góc/i.test(x)));
kt("§18 KHÔNG định danh máy nào lọt lên bề mặt", "",
   !/plane_perpendicular_to_line|mpb|construct_/.test(t.chu));
...
kt("§35 số đo dẫn xuất hiện đúng trên dải kết quả", JSON.stringify(t.ketQua),
   t.ketQua.length === 1 && /Khoảng cách/i.test(t.ketQua[0]));

/* Chọn SAU khi tua tới bước cuối: một vật chỉ có mặt trong cảnh từ bước
 * dựng nó trở đi, nên chọn nó ở bước 1 thì không có gì để soi — bất biến
 * #31 (`frame k ⇔ trace[k]`) áp đúng ở đây. */
...
await sleep(800);
t = await T();
kt("§18 ô soi mở đúng vật và nói được nó dựng từ đâu",
   JSON.stringify([t.soiTen, t.soiVai]),
   /vuông góc/i.test(t.soiTen || "") && (t.soiVai || "").length > 0);
